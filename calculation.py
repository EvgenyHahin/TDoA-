...
class TDoAEquationsWindow:
    """Window displaying TDoA hyperbolic equations and their graphical representation."""
    ...
    def __init__(self, parent, reference_station, stations, station_coords, error_ns,
        ...
        self.C_ns = 0.299792458  # Distance traveled by light in 1 ns (m)
        self.is_open = True
    ...
    def create_widgets(self):
        """Create all UI elements for the equations window."""
        ...
        # Prepare header information with coordinates and error if available
        ...
        # Add button to plot hyperbolas if we have packet data
        ...
        # Main scrollable area for equation content
    ...
    def create_equations_content(self, parent):
        """Create the detailed content showing equations and calculations."""
        ref_x, ref_y = self.station_coords.get(self.reference_station, (0, 0))

        # Section for packet-specific data
        if self.packet_data:
            ...
            # Add true and calculated coordinates if available
            if self.true_uav_coords:
                x, y = self.true_uav_coords
        ...
        # Section for reception times and errors table
        ...
        # Section for general hyperbola equation
        ...
        # Section for specific equations for each station pair
        ...
        for station in self.stations:
            ...
            # Create equation card for this station
            ...
            # Calculate and display if we have reception times
            if t_i is not None and t_ref is not None:
                ...
                # Save hyperbola data for plotting
                ...
                # Verify with true UAV position if available
                if self.true_uav_coords:
                    x_true, y_true = self.true_uav_coords
        ...
        # Add geometric interpretation section if we have data
    ...
    def plot_hyperbolas(self):
        """Plot hyperbolas for TDoA positioning on a map with stations and UAV positions."""
        ...
        # Determine plot limits based on station and UAV positions
        all_x = []
        all_y = []
        for station, (x, y) in self.station_coords.items():
            ...
            all_y.append(y)
        if self.true_uav_coords:
            all_x.append(self.true_uav_coords[0])
            all_y.append(self.true_uav_coords[1])
        if self.calculated_coords:
            all_x.append(self.calculated_coords[0])
            all_y.append(self.calculated_coords[1])
        ...
        # Plot stations with different markers for reference station
        ...
        # Plot UAV positions if available
        if self.true_uav_coords:
            x_true, y_true = self.true_uav_coords
        ...
        if self.calculated_coords:
            ...
            # Plot error line between true and calculated positions
        ...
        # Plot hyperbolas for each station pair
        problem_stations = []
        ...
        for i, hyperbola in enumerate(self.hyperbolas_data):
            ...
            # Skip if hyperbola condition is not met
            ...
            # Generate points for hyperbola branches
            ...
            # Rotate and shift hyperbola to correct position
            ...
            # Determine which branch is active based on delta_d sign
            test_idx = len(t) // 2
            ...
            # Plot the active branch with solid line, inactive with dashed
            if is_right_active:
    ...
    def on_mouse_press(self, event):
        ...
        if event.inaxes == self.ax and event.button == 3:  # Right click
    ...
    def create_section_frame(self, parent, title):
        """Create a styled section frame with a title header."""
...
class UAVDataViewer:
    """
    Main application class for UAV TDoA positioning analysis.
    Provides GUI interface for loading data, calculating positions,
    and analyzing accuracy metrics like GDOP and CRLB.
    """
    ...
    def build_covariance_matrix_Q(sigma_ns, N):
        """Build covariance matrix for TDoA measurements."""
        c = 299792458.0
    ...
    def compute_CRLB_for_active_stations(self, x, y, sigma_t_ns, active_stations, reference_station):
        """
        Compute Cramér-Rao Lower Bound (CRLB) for position estimation error.
        Returns standard deviations for x and y coordinates.
        """
    ...
    def generate_GDOP_map(self, grid_size=100, reference_station=None):
        """
        Generate GDOP (Geometric Dilution of Precision) map for visualization.
        Returns grid coordinates and GDOP values.
        """
        ...
        # Determine area bounds based on station coordinates
        ...
        # Calculate GDOP for each grid point
    ...
    def calculate_GDOP_statistics(self, reference_station=None):
        """
        Calculate GDOP statistics along the UAV trajectory.
        Returns mean, max, min, std, and percentile statistics.
        """
        ...
        stats = {
            'mean': np.mean(gdop_values),
            'max': np.max(gdop_values),
            'min': np.min(gdop_values),
    ...
    def show_gdop_map(self):
        ...
        # Set default reference station if available
        if self.reference_station:
            ref_station_var.set(f"Станция {self.reference_station}")

        # Option to show maps for all stations
    ...
    def plot_gdop_map(self, window, ref_station_var, show_all_var):
        """Generate and display GDOP map based on user selection."""
        # Clear previous content
        for widget in self.gdop_canvas_frame.winfo_children():
            widget.destroy()

        # Get UAV true positions for trajectory overlay
        true_x, true_y = [], []
        ...
        # Create figure based on selection (single station or all stations)
        if show_all_var.get():
            self._plot_all_stations_gdop_map(true_x, true_y)
        else:
            self._plot_single_station_gdop_map(ref_station_var, true_x, true_y)

    def _plot_all_stations_gdop_map(self, true_x, true_y):
        """Plot GDOP maps for all reference stations in a grid layout."""
        ...
        # Adjust axes array shape for consistent handling
        if num_stations == 1:
            axes = np.array([[axes]])
        ...
        # Generate GDOP maps for all stations
        ...
        if all_gdop_values:
            ...
            self._embed_plot_in_window(fig)

    def _plot_single_station_gdop_map(self, ref_station_var, true_x, true_y):
        """Plot GDOP map for a single selected reference station."""
        ...
        if selected_text:
            ...
            if X is not None:
                ...
                # Plot GDOP map with logarithmic color scale
                im = ax.imshow(GDOP_plot, extent=[X.min(), X.max(), Y.min(), Y.max()], origin='lower',
                ...
                # Plot UAV trajectory with start and end markers
                if true_x and true_y:
                ...
                # Display GDOP statistics
                gdop_stats = self.calculate_GDOP_statistics(station_num)
                if gdop_stats:
                    self._display_gdop_statistics(station_num, gdop_stats)
            else:
    ...
    def _embed_plot_in_window(self, fig):
        """Embed matplotlib figure into Tkinter window with toolbar."""
    ...
    def _display_gdop_statistics(self, station_num, gdop_stats):
        """Display GDOP statistics for the selected station."""
    ...
    # Additional methods for time-based GDOP plots, CRLB analysis, etc.
    # (Similar structured methods would follow here...)

    def create_widgets(self):
        """Create and arrange all UI widgets for the main application window."""
        # Control panel with scrollable horizontal layout
        ...
        # Create all control buttons and inputs
    ...
    def _create_control_widgets(self):
        """Create all control widgets in the control panel."""
        # Load button
    ...
    def _create_info_display(self):
        """Create information display area below the control panel."""
    ...
    def _create_data_table(self):
        """Create scrollable data table for displaying measurements and results."""
    ...
    def load_json_file(self):
        ...
        try:
            ...
            # Close any open equations window
    ...
    def apply_error(self):
        """Apply measurement error parameters to the analysis."""
        try:
            # Get error parameters from UI
    ...
    def update_settings_label(self):
        """Update settings label with current parameters."""
    ...
    def calculate_tdoa_positions(self):
        """Calculate UAV positions using TDoA method with Monte Carlo simulations."""
        ...
        for packet_id in packet_ids:
            ...
            # Get reception times for all stations
            reception_times = {}
            for station in self.stations:
            ...
            for _ in range(self.num_trials):
                ...
                # Estimate position using TDoA with noisy measurements
                ...
                # Calculate CRLB for active stations
            ...
            # Store average results for this packet
            if errors:
    ...
    def tdoa_position_estimation_noisy_with_active(self, reception_times):
        """
        Estimate UAV position using TDoA measurements with error handling.
        Returns (position, list of active stations used in calculation).
        """
        # Validate reference station data
        ...
        # Prepare data for available stations
        available_stations = []
        time_diffs_list = []
        station_coords_list = []
        ...
        # Need at least 2 stations for 2D positioning
        ...
        # Initial guess as centroid of stations
        initial_guess = np.mean(np.vstack([ref_coords, station_coords_arr]), axis=0)

        # Define error function for least squares optimization
        ...
        # Try least squares optimization first
        try:
        ...
        # Fallback to Chan algorithm if least squares fails
    ...
    def chan_algorithm(self, ref_coords, station_coords, time_diffs, sigma_ns):
        """
        Implementation of Chan's algorithm for TDoA positioning.
        A closed-form solution that is more robust than iterative methods.
        """
        c = self.C
        N = len(time_diffs)
        ...
        try:
            ...
            # Calculate measured distances and geometry vectors
            d_meas = c * time_diffs
            dx = x_i - x_ref
            dy = y_i - y_ref
            ...
            # Apply weighting based on measurement error
            ...
            # Fallback to unweighted least squares
    ...
    def show_trajectory(self):
        """Display UAV trajectory with true and estimated positions."""
        ...
        # Apply smoothing to estimated positions
    ...
    def show_errors(self):
        """Display error analysis for position estimation."""
        # Validate data availability
        if not self.calculated_positions:
            messagebox.showwarning("Внимание", "Нет вычисленных координат")
            return
        ...
        std_of_std = np.nanstd(std_errors)  # Variation of precision along trajectory

        # Create error analysis window
...
"""
    ...
    def analyze_reference_stations(self):
        """Analyze positioning accuracy for different reference stations."""
        ...
        # Calculate errors for all reference stations
    ...
    def calculate_errors_for_all_references(self):
        ...
        for ref_station in self.stations:
            ...
            station_errors = []  # ВСЕ ОШИБКИ ПО ВСЕМ ПАКЕТАМ ДЛЯ ЭТОЙ СТАНЦИИ
            ...
            for packet_id in packet_ids:
                ...
                packet_errors = []  # ОШИБКИ ТОЛЬКО ДЛЯ ТЕКУЩЕГО ПАКЕТА
                for _ in range(min(self.num_trials, 50)):
                    ...
                    calc, _ = result
                    err = math.sqrt((calc[0] - true_x) ** 2 + (calc[1] - true_y) ** 2)
                    packet_errors.append(err)

                if packet_errors:
                    station_errors.extend(packet_errors)  # ДОБАВЛЯЕМ ОШИБКИ ПО ПАКЕТУ К ОБЩИМ

            # ВОССТАНАВЛИВАЕМ ИСХОДНУЮ ОПОРНУЮ СТАНЦИЮ
            self.reference_station = current_ref

            if station_errors:
                # ВЫЧИСЛЯЕМ СТАТИСТИКУ ПО ВСЕМ ОШИБКАМ ДЛЯ СТАНЦИИ
    ...
    def plot_reference_station_analysis(self, results):
        """Create comprehensive visualization of reference station analysis results."""
        # Filter out stations with no valid results
        ...
        # Plot 2: Comparison of error metrics for top stations
        ...
        # Plot 3: Error distribution for best stations
        ...
        # Plot 4: Summary information panel
...
"""
    ...
    def save_analysis_results(self, results):
        """Save reference station analysis results to CSV or Excel file."""
        file_path = filedialog.asksaveasfilename(
    ...
    def select_best_station(self, station_num):
        """Select the specified station as the reference station."""
    ...
    def show_tdoa_equations_selected(self):
        """Show TDoA equations for the selected packet."""
    ...
    def on_row_double_click(self, event):
        """Handle double-click on table row to show equations."""
        self.show_equations_for_selected_packet()

    def show_equations_for_selected_packet(self):
        """Display TDoA equations window for the selected packet."""
        selected_item = self.tree.selection()
        ...
        # Generate random errors for display
        station_errors_ns = {}
        for station in self.stations:
            if station != self.reference_station:
                station_errors_ns[station] = np.random.normal(0, self.error_ns)

        # Close existing equations window if open
        if self.equations_window:
            self.equations_window.destroy()
            self.equations_window = None

        # Create and show equations window
        try:
    ...
    def update_table(self):
        """Update table with current settings."""
        if self.reference_station:
            self.display_data(show_differences=True)
        else:
            messagebox.showwarning("Внимание", "Выберите опорную станцию")
    ...
    def display_data(self, show_differences=True):
        """Display data in the table with current settings."""
        # Clear existing data
        ...
        for packet_id in packet_ids:
            ...
            # Add differences and errors if reference station is selected
            if show_differences and self.reference_station:
                ...
                for station in self.stations:
                    ...
                    # Generate random error for display
                ...
                for packet_id in packet_ids:
                    ...
                    if self.reference_station:
                        ...
                        for s in self.stations:
                            ...
                            # For export, use one random realization
                            eps = np.random.normal(0, self.error_ns)
                            row.append(str(eps))
